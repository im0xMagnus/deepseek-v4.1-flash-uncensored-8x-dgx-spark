...
for r in $TARGETS; do
  ...
  $J "$h" "set -e; mkdir -p /data/models/.images; cd /data/models/.images; curl -sSf -o $B.sha256 http://$SRC_IP:$HTTP_PORT/$B.sha256; curl -sS --retry 5 -C - -o $B http://$SRC_IP:$HTTP_PORT/$B; echo \"\$(cat $B.sha256)  $B\" | sha256sum -c - && docker load -i $B | tail -1 && docker images $IMG --format '  loaded {{.Repository}}:{{.Tag}} {{.ID}}'" &
done
wait
