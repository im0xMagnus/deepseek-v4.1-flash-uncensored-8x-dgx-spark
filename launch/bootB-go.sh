...
export EXP_NAME=bootB IMAGE=vllm-dsv41:overlay5 \
       GMU="${GMU:-0.75}" MAXLEN=1048576 SEQS=8 MAX_BATCHED=8192 LONG_PREFILL="${LONG_PREFILL:-2048}" EAGER=0 CUDAGRAPH_MODE=FULL_AND_PIECEWISE \
...
echo "$EXP_NAME go $(date -u +%FT%TZ) gmu=$GMU maxlen=$MAXLEN long_prefill=$LONG_PREFILL spec=$SPEC k=$SPEC_K eager=$EAGER cg=$CUDAGRAPH_MODE ep=$EP"
