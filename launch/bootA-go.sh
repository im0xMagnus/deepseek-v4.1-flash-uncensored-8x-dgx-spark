...
# GMU 0.75 not 0.80: at eight ranks the NCCL communicators take ~24 GiB per rank before the model loads, so only
# 96-97 of 121.7 GiB is free at vLLM's startup check and 0.80 (97.35 GiB) is refused (boot A attempts 2-3); 0.77 served but left
# the head at ~1.1 GiB free under a 500K prefill, so 0.75 buys 2.4 GiB of headroom per node for ~600K tokens of pool.
export EXP_NAME=bootA IMAGE=vllm-dsv41:overlay5 \
       GMU="${GMU:-0.75}" MAXLEN=300000 SEQS=8 MAX_BATCHED=8192 EAGER=0 CUDAGRAPH_MODE=FULL_AND_PIECEWISE \
